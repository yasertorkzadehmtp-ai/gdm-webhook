...
# =========================================================
# GDM Server Render v3-i6c — Telegram Sanitize (inline LOG) — Base: v3-i5 Reliability Polish
# Purpose:
...
def strip_log_from_body(raw_body: str) -> str:
    """
    If raw_body contains telemetry, remove it and return only the human signal text.

    Engine conventions supported:
      1) <signal text>\nLOG:{...json...}
      2) LOG:{...json...}
      3) <signal text> LOG:{...json...}   (inline LOG appended to last line)
    """
    if not raw_body:
        return ""

    # If LOG is appended inline, strip everything from ' LOG:{' onward.
    if " LOG:{" in raw_body:
        raw_body = raw_body.split(" LOG:{", 1)[0].rstrip()
...
def parse_log_json(raw_body: str) -> Optional[Dict[str, Any]]:
    """
    Extract JSON after 'LOG:' if present (line-start or inline). Returns dict or None.
    """
    if not raw_body:
        return None
    ...
    # 2) Inline ' LOG:{...}' appended to a signal line
    if " LOG:{" in raw_body:
        payload = "{" + raw_body.split(" LOG:{", 1)[1]
        try:
            obj = json.loads(payload)
            if isinstance(obj, dict):
                return obj
        except Exception:
            logger.exception("Failed to parse inline LOG JSON")
        return None
